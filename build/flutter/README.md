...
* `arq_metabolica_mx` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `arq-metabolica-mx` - executable or bundle name on disk.
* `ARQ-Metabolica MX` - display name shown in window titles and about app dialogs.
* `mx.arqmetabolica` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
